...
export default function Search() {
  ...
  const doSearch = async () => {
    ...
    try {
      ...
      // Fetch alerts for the results to show warnings
    ...
    <Box>
      ...
      <Box bg="surface.card" border="1px solid" borderColor="surface.border" borderRadius="14px" p={6} mb={6}>
        ...
        <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={4} mb={4}>
          <FormControl>
            <FormLabel fontSize="xs" color="#64748b">Tracking ID</FormLabel>
            <Input placeholder="Tracking ID (e.g. TRK-...)" value={params.tracking_id || ''} onChange={e => set('tracking_id', e.target.value)} size="sm" />
          </FormControl>
          <FormControl>
            <FormLabel fontSize="xs" color="#64748b">General Search</FormLabel>
            <Input placeholder="Search anything..." value={params.q || ''} onChange={e => set('q', e.target.value)} size="sm" />
          </FormControl>
          <FormControl>
            <FormLabel fontSize="xs" color="#64748b">BL Number</FormLabel>
            <Input placeholder="BL number" value={params.bl_number || ''} onChange={e => set('bl_number', e.target.value)} size="sm" />
          </FormControl>
          <FormControl>
            <FormLabel fontSize="xs" color="#64748b">Container Number</FormLabel>
            <Input placeholder="Container number" value={params.container_number || ''} onChange={e => set('container_number', e.target.value)} size="sm" />
          </FormControl>
          <FormControl>
            <FormLabel fontSize="xs" color="#64748b">Vessel Name</FormLabel>
            <Input placeholder="Vessel name" value={params.vessel_name || ''} onChange={e => set('vessel_name', e.target.value)} size="sm" />
          </FormControl>
          <FormControl>
            <FormLabel fontSize="xs" color="#64748b">Customer Name</FormLabel>
            <Input placeholder="Customer name" value={params.customer_name || ''} onChange={e => set('customer_name', e.target.value)} size="sm" />
          </FormControl>
          <FormControl>
            <FormLabel fontSize="xs" color="#64748b">Status</FormLabel>
        ...
        <HStack>
          <Button leftIcon={<FiSearch />} size="sm" onClick={doSearch} isLoading={loading}>Search</Button>
      ...
      {searched && (
        <Box bg="surface.card" border="1px solid" borderColor="surface.border" borderRadius="14px" overflow="hidden">
          ...
          {loading ? (
            <Flex justify="center" py={12}><Spinner size="lg" color="brand.400" /></Flex>
          ) : (
            <Box overflowX="auto">
              <Table variant="simple" size="sm">
                ...
                <Tbody>
                  {results.length === 0 ? (
                    <Tr><Td colSpan={8} textAlign="center" color="#64748b" py={8}>No results found. Try different search criteria.</Td></Tr>
                  ) : results.map(s => (
                    <Tr key={s.id} _hover={{ bg: 'surface.cardHover' }}>
                      <Td fontWeight="600" color="white">{s.shipment_number || '—'}</Td>
                      <Td color="#cbd5e1">
                         <Text fontWeight="500">{s.customer_name || '—'}</Text>
                      </Td>
                      <Td>
                        {parseInt(String(s.activeAlerts || 0)) > 0 ? (
                          <HStack color="#ef4444" spacing={1}>
                            <FiAlertCircle />
                            <Text fontSize="xs" fontWeight="bold">{s.activeAlerts}</Text>
                          </HStack>
                        ) : (
